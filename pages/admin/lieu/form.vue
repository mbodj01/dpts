  ...
  export default {
    ...
    methods: {
      ...
      async handleForm() {
        try {
          if (this.editId) {
            await this.$axios.$put(`/lieu_signalisation/update/${this.editId}`, {
              ...this.payload,
            })
          } else {
            await this.$axios.$post('/lieu_signalisation/create', {
              ...this.payload,
            })
          }
          this.$router.push('/admin/lieu')
        } catch ({ response }) {
      ...
      async getLieu() {
        try {
          this.payload = await this.$axios.$get(`/lieu_signalisation/show/${this.editId}`)
        } catch ({ response }) {
          this.error = response.data
        }
      },
